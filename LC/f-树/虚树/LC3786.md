### Title [3786. 树组的交互代价总和](https://leetcode.cn/problems/total-sum-of-interaction-cost-in-tree-groups/)
...
public:
    long long interactionCosts(int n, vector<vector<int>>& edges, vector<int>& group) {
        ...
        for (int tag = 1; tag <= tags; tag++) {
            ...
            // 基于虚树进行树形DP
            auto& gvt = vt.VT[tag];
            auto dfs_t1 = [&](auto&& dfs_t1, int u, int o) -> int {
                int size = 0;
                if (group[u] == tag) size = 1;
                for (auto& v : gvt[u]) if (v != o) {
                    int sz = dfs_t1(dfs_t1, v, u);
