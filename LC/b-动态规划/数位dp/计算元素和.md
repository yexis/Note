...
##### 2 key points
 ...
 数位DP一般计算一段区间$[start, end]$之间满足条件的元素个数，这种使用常规的数位DP即可



另一种所求为区间$start, end$之间满足条件的元素的和



##### 3 thought

在数位DP中，记忆化的部分一定是 $ !is\_limit \ \ \ \&\& \ \ \ is\_num  $ 的条件下；$is\_limit$ 和 $!is\_num$都是单独计算

且在数位DP中，左边($idx$小)的为高位，，当前位对应的值是 $power(x, m - i - 1)$， 当遇到需要用到当前位的位数来统计答案时会用到
